...
function AboutPage() {
  return (
    <div className={styles.aboutPage}>
      <h2>About This Word Builder App</h2>
      <p>
        This is a fun **Word Builder application** designed for first graders,
        built with React and demonstrating fundamental web development concepts
        with the help of CTD. It interacts with Airtable as a backend for
        storing and accessing the words kids can create and practice.
      </p>
      ...
      <p>
        **Repositories** like GitHub are used for **version control** and
        **collaboration**.
      </p>

      <h4>2. Project Structure and Components:</h4>
      <p>
        React applications are organized into a **component-based
        architecture**. Common directories include: `src/` (all source code),
        `src/components/` (reusable UI elements), `src/features/` (components
        related to specific application features), `src/pages/` (top-level
        components representing different views/pages), `src/shared/` (utility
        components or styles shared across the application), `src/assets/`
        (static assets like images), and `src/reducers/` (logic for managing
        complex state, often used with `useReducer`). Components are JavaScript
        functions that return React elements, describing what should appear on
        the screen.
      </p>
      ...
      <p>
        ...
        feature-specific state management, as seen with the `words.reducer.js`
        in this app.
      </p>
      ...
      <h4>7. Pagination:</h4>
      <p>
        The word list implements **client-side pagination**, displaying a
        limited number of items per page. This is achieved by calculating the
        subset of words to display based on the current page and items per page.
        It uses `useSearchParams` to store the current page number in the URL's
        query parameters, allowing direct navigation to specific pages and
        maintaining state on refresh. It provides "Previous" and "Next" buttons
        to update the `page` parameter in the URL. It also implements logic to
        handle invalid page numbers in the URL, redirecting to the first page if
        necessary.
      </p>

      <p>
        This application serves as a practical example of building a
        **single-page application (SPA)** with React's core features and popular
        libraries.
      </p>
    </div>
  );
